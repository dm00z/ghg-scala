...
import model.DirectTable.{PoolData, PrimaryPoolData, GenericData}
import model.GasData.{GasRatioTable, GWP}
import GasRatioTable.{Group => GasGroup, Row => GasRow}
import GWP.{Row => GWPRow}
import model._, model.ElectricData.{CountryPowerStruct, CalcMethod, PowerSupply}
